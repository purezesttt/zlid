...
local function CreateUILibrary()
  ...
  local function LoadUI(scriptName, subtitleText)
    ...
    ScreenGui.Parent = LocalPlayer:WaitForChild("PlayerGui") -- Parent to PlayerGui immediately
    ...
    local UI_API = {}
    UI_API.Tabs = {}
    UI_API.ScreenGui = ScreenGui -- Expose the ScreenGui for potential direct manipulation
    ...
    function UI_API.CreateTab(name)
      ...
      Button.MouseButton1Click:Connect(function()
        ...
        -- Update CanvasSize of TabContentFrame
        ...
        TabContentFrame.CanvasSize = UDim2.new(0, 0, 0, contentHeight)
      end)
    ...
    function UI_API.CreateDropdown(parent, config)
      ...
      if config.CurrentSelection then
        if type(config.CurrentSelection) == "table" then
          -- Deep copy the table
          for k, v in pairs(config.CurrentSelection) do
            CurrentSelectedOptions[k] = v
          end
        else
