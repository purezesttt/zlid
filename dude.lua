...
local function CreateUILibrary()
    ...
    local function LoadUI(scriptName, subtitleText)
        ...
        ScreenGui.Name = scriptName or "ok"
        ...
        function UI_API.CreateTab(name)
            ...
            Button.MouseButton1Click:Connect(function()
                if CurrentActiveTabContent then
                    local prevTabButton = TabButtons[CurrentActiveTabContent.Name]
                    if prevTabButton then
                        prevTabButton.BackgroundColor3 = Color3.fromRGB(35, 35, 35)
                        prevTabButton.TextColor3 = Color3.fromRGB(200, 200, 200)
                    end
                ...
                CurrentActiveTabContent = TabContent

                task.defer(function()
                    local contentHeight = ContentListLayout.AbsoluteContentSize.Y + UIPaddingContent.PaddingTop.Offset + UIPaddingContent.PaddingBottom.Offset
                    TabContentFrame.CanvasSize = UDim2.new(0, 0, 0, math.max(TabContentFrame.AbsoluteSize.Y, contentHeight))
                end)
            end)

            -- Activate first tab if none is active
            if not CurrentActiveTabContent then
        ...
        function UI_API.CreateDropdown(parent, config)
            ...
            if config.CurrentSelection then
                if type(config.CurrentSelection) == "table" then
                    for _, v in ipairs(config.CurrentSelection) do
                        table.insert(CurrentSelectedOptions, v)
                    end
                else
            ...
            OptionsFrame.Size = UDim2.new(1, 0, 0, #config.Options * 27 + (OptionsListLayout.Padding.Offset * (#config.Options - 1)))
    ...
    function Library:CreateWindow(config)
        local windowName = config.Name or "UI Window"
        local subtitle = config.Subtitle or ""

        if LoadedWindows[windowName] then
            warn("Window '" .. windowName .. "' already exists. Returning existing instance.")
            return LoadedWindows[windowName]
        end

        local UI_API_Instance = LoadUI(windowName, subtitle)
        LoadedWindows[windowName] = UI_API_Instance
        return UI_API_Instance
    end
